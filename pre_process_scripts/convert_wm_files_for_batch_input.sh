...
awk -v mcd=$max_cos_dist 'BEGIN { FS="\t";  OFS="\t";  } { if ($3 <= mcd) print $1,$2,"SIM_NOUN",$3}' $noun_noun_sims > $output_dir/noun_sim_rels.csv
...
awk -v mcd=$max_cos_dist 'BEGIN { FS="\t";  OFS="\t"} { if ($3 <= mcd) print $1,$2,"SIM_STAT",$3}' $stat_stat_sims  > $output_dir/stat_sim_rels.csv
